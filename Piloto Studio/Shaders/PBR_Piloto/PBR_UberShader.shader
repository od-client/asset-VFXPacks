...
Shader "Piloto Studio/PBR_UberShader"
{
	Properties
	{
		...
		_ExposureWeight("Exposure Weight", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 uv_OcclusionRoughtMetal = packedInput.ase_texcoord5.xy * _OcclusionRoughtMetal_ST.xy + _OcclusionRoughtMetal_ST.zw;
				...
				float2 uv_PanningMask = packedInput.ase_texcoord5.xy * _PanningMask_ST.xy + _PanningMask_ST.zw;
				float2 panner25 = ( 1.0 * _Time.y * _PanningSpeed + uv_PanningMask);
				float3 hdEmission18 = ASEGetEmissionHDRColor(( ( ( _AdditiveMaskEdge * ( tex2DNode2 * _EmissiveTint ) ) * 0.5 ) + ( _PanningNoiseStrenght * ( tex2DNode2 * ( _EmissiveTint * tex2D( _PanningMask, panner25 ).r ) ) ) ).rgb,_Intensity,_ExposureWeight,GetInverseCurrentExposureMultiplier());
				
				surfaceDescription.BaseColor = ( _BaseColorTint * tex2DNode1 ).rgb;
				surfaceDescription.Normal = unpack3;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = ( _MetallicScale * tex2DNode4.b );

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = 0;
				#endif

				surfaceDescription.Emission = hdEmission18;
				surfaceDescription.Smoothness = ( saturate( ( 1.0 - tex2DNode4.g ) ) * _SmoothnessScale );
				surfaceDescription.Occlusion = ( tex2DNode4.r * _OcclusionScale );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_OcclusionRoughtMetal = packedInput.ase_texcoord2.xy * _OcclusionRoughtMetal_ST.xy + _OcclusionRoughtMetal_ST.zw;
				float4 tex2DNode4 = tex2D( _OcclusionRoughtMetal, uv_OcclusionRoughtMetal );
				
				float2 uv_Emission = packedInput.ase_texcoord2.xy * _Emission_ST.xy + _Emission_ST.zw;
				float4 tex2DNode2 = tex2D( _Emission, uv_Emission );
				float2 uv_PanningMask = packedInput.ase_texcoord2.xy * _PanningMask_ST.xy + _PanningMask_ST.zw;
				float2 panner25 = ( 1.0 * _Time.y * _PanningSpeed + uv_PanningMask);
				float3 hdEmission18 = ASEGetEmissionHDRColor(( ( ( _AdditiveMaskEdge * ( tex2DNode2 * _EmissiveTint ) ) * 0.5 ) + ( _PanningNoiseStrenght * ( tex2DNode2 * ( _EmissiveTint * tex2D( _PanningMask, panner25 ).r ) ) ) ).rgb,_Intensity,_ExposureWeight,GetInverseCurrentExposureMultiplier());
		...
		Pass
		{
			...
			UNITY_SETUP_INSTANCE_ID(packedInput);
				...
				float2 uv_OcclusionRoughtMetal = packedInput.ase_texcoord3.xy * _OcclusionRoughtMetal_ST.xy + _OcclusionRoughtMetal_ST.zw;
				float4 tex2DNode4 = tex2D( _OcclusionRoughtMetal, uv_OcclusionRoughtMetal );
		...
		Pass
		{
						...
						)
			{
				...
				float2 uv_OcclusionRoughtMetal = packedInput.ase_texcoord7.xy * _OcclusionRoughtMetal_ST.xy + _OcclusionRoughtMetal_ST.zw;
				float4 tex2DNode4 = tex2D( _OcclusionRoughtMetal, uv_OcclusionRoughtMetal );
				
				float2 uv_Emission = packedInput.ase_texcoord7.xy * _Emission_ST.xy + _Emission_ST.zw;
				float4 tex2DNode2 = tex2D( _Emission, uv_Emission );
				float2 uv_PanningMask = packedInput.ase_texcoord7.xy * _PanningMask_ST.xy + _PanningMask_ST.zw;
				float2 panner25 = ( 1.0 * _Time.y * _PanningSpeed + uv_PanningMask);
				float3 hdEmission18 = ASEGetEmissionHDRColor(( ( ( _AdditiveMaskEdge * ( tex2DNode2 * _EmissiveTint ) ) * 0.5 ) + ( _PanningNoiseStrenght * ( tex2DNode2 * ( _EmissiveTint * tex2D( _PanningMask, panner25 ).r ) ) ) ).rgb,_Intensity,_ExposureWeight,GetInverseCurrentExposureMultiplier());
...
WireConnection;5;4;55;0
WireConnection;5;6;18;0
WireConnection;5;7;52;0
WireConnection;5;8;47;0
